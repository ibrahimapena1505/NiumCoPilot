...
const PIPELINE = [
  {
    title: 'Web Scraper',
    subtitle: 'BeautifulSoup + Playwright',
    description: 'Collects Nium documentation with dynamic rendering support.',
  },
  {
    title: 'Knowledge Base',
    subtitle: 'SQLAlchemy ORM',
    description: 'Normalizes content into structured chunks with metadata.',
  },
  {
    title: 'AI Assistant',
    subtitle: 'OpenAI GPT Integration',
    description: 'Retrieves relevant context and crafts guided responses.',
  },
];
    ...
    <Box sx={{ mt: 4 }}>
      ...
      <Grid2 container spacing={2} columns={12}>
        {PIPELINE.map((stage, index) => (
          <Grid2 key={stage.title} xs={12} md={4}>
            <Card sx={{
      ...
      <Grid2 container spacing={2} columns={12} sx={{ mt: 2 }}>
        {DOWNSTREAM.map(([title, subtitle]) => (
          <Grid2 key={title} xs={12} sm={6} md={4}>
